...
async function updateStreak(userId: string): Promise<void> {
  const date = today();
  // Best-effort lock key prevents double-increment when multiple sections submit in parallel
...
routes.get('/daily-config', async (c) => {
  ...
  // Build content bank if missing (check ANY bank key, not just Showerthoughts)
  const bankExists = (await redis.get('ContentBank:AskReddit')) || (await redis.get('ContentBank:Showerthoughts'));
  if (!bankExists) {
...
routes.post('/mood/submit', async (c) => {
  ...
  const submitted = await redis.hGet(`MoodSubmitted:${date}`, userId);
  if (submitted) {
...
routes.post('/choice/submit', async (c) => {
  ...
  const submitted = await redis.hGet(`ChoiceSubmitted:${date}:${body.questionId}`, userId);
  if (submitted) {
...
routes.post('/template/submit', async (c) => {
  ...
  const submitted = await redis.hGet(`TemplateSubmitted:${date}:${body.templateId}`, userId);
  if (submitted) {
...
routes.post('/confession/vote', async (c) => {
  ...
  const submitted = await redis.hGet(`ConfessionSubmitted:${date}:${body.confessionId}`, userId);
  if (submitted) {
...
routes.get('/confessions/feed', async (c) => {
  ...
  const date = today();
  const raw = await redis.get(`ConfessionFeed:${date}`);
  const confessions: CommunityConfession[] = raw ? JSON.parse(raw) as CommunityConfession[] : [];
...
routes.post('/prompts/submit', async (c) => {
  ...
  const submission = { id, userId, type: body.type, payload: body.payload, status: 'pending' };
  await redis.set(`PromptSubmission:${id}`, JSON.stringify(submission));
  await redis.zAdd('PromptSubmissionIds', { member: id, score: Date.now() });
...
routes.post('/prompts/moderate', async (c) => {
  ...
  const body = await c.req.json<{ submissionId: string; action: 'approve' | 'reject' }>();
...
routes.post('/results/comment', async (c) => {
  ...
  try {
    ...
    const now = Date.now();
    const msUntilMidnight = new Date(date).getTime() + 86400000 - now;
    await redis.set(lockKey, '1');
    await redis.expire(lockKey, Math.max(60, Math.ceil(msUntilMidnight / 1000)));
...
routes.get('/weekly/recap', async (c) => {
  ...
  return c.json<WeeklyRecapResponse>({
    weekEnding,
    totalDaysPlayed,
    dominantMood,
    topChoice,
...
routes.post('/internal/seed-demo', async (c) => {
  try {
    ...
    return c.json<ErrorResponse>({ status: 'error', message: 'Seed failed' }, 500);
  }
});
